...
class JwtAuthenticationFilterTest {
    ...
    class PublicPaths {
        ...
        void shouldPassAuthRegisterWithoutToken() {
            ...
            MockServerWebExchange exchange = MockServerWebExchange.from(MockServerHttpRequest.get("/auth/register").build());
            StepVerifier.create(filter.filter(exchange, chain)).verifyComplete();
            verify(jwtUtil, never()).extractClaims(any());
        }

        @Test
        @DisplayName("deve passar subpaths de rotas públicas")
        void shouldPassSubpathsOfPublicPaths() {
            when(chain.filter(any())).thenReturn(Mono.empty());
            MockServerWebExchange exchange = MockServerWebExchange.from(MockServerHttpRequest.get("/auth/register/confirm").build());
            StepVerifier.create(filter.filter(exchange, chain)).verifyComplete();
    ...
    class MissingOrInvalidHeader {
        ...
        void shouldReturn401WhenHeaderDoesNotStartWithBearer() {
            MockServerWebExchange exchange = MockServerWebExchange.from(
                    MockServerHttpRequest.get("/orders").header(HttpHeaders.AUTHORIZATION, "Basic 123").build());
            StepVerifier.create(filter.filter(exchange, chain)).verifyComplete();
            assertThat(exchange.getResponse().getStatusCode()).isEqualTo(HttpStatus.UNAUTHORIZED);
        }
    }
    ...
    class InvalidToken {
        ...
        void shouldReturn401WhenTokenIsInvalid() {
            when(jwtUtil.extractClaims("token-invalido")).thenThrow(new RuntimeException("JWT inválido"));
            MockServerWebExchange exchange = MockServerWebExchange.from(
                    MockServerHttpRequest.get("/orders").header(HttpHeaders.AUTHORIZATION, "Bearer token-invalido").build());
            StepVerifier.create(filter.filter(exchange, chain)).verifyComplete();
            assertThat(exchange.getResponse().getStatusCode()).isEqualTo(HttpStatus.UNAUTHORIZED);
        }
        ...
        void shouldReturn401WhenTokenIsExpired() {
            when(jwtUtil.extractClaims("token-expirado")).thenThrow(new ExpiredJwtException(null, null, "Token expirado"));
            MockServerWebExchange exchange = MockServerWebExchange.from(
                    MockServerHttpRequest.get("/payments/user").header(HttpHeaders.AUTHORIZATION, "Bearer token-expirado").build());
            StepVerifier.create(filter.filter(exchange, chain)).verifyComplete();
            assertThat(exchange.getResponse().getStatusCode()).isEqualTo(HttpStatus.UNAUTHORIZED);
        }
    }
    ...
    @Nested
    @DisplayName("autorização (RBAC)")
    class Authorization {
        @Test
        @DisplayName("deve retornar 403 quando USER tenta acessar rota ADMIN")
        void shouldReturn403WhenUserAccessAdminRoute() {
            Claims claims = mock(Claims.class);
            when(jwtUtil.extractClaims(anyString())).thenReturn(claims);
            when(jwtUtil.extractRole(claims)).thenReturn("USER");
            MockServerWebExchange exchange = MockServerWebExchange.from(
                    MockServerHttpRequest.get("/orders/all").header(HttpHeaders.AUTHORIZATION, "Bearer tk").build());
            StepVerifier.create(filter.filter(exchange, chain)).verifyComplete();
    ...
    class ValidToken {
        @Test
        @DisplayName("deve propagar X-Auth-Username e X-Auth-Role")
        void shouldPropagateAuthHeadersDownstream() {
            ...
            MockServerWebExchange exchange = MockServerWebExchange.from(
                    MockServerHttpRequest.get("/orders").header(HttpHeaders.AUTHORIZATION, "Bearer token-valido").build());

            StepVerifier.create(filter.filter(exchange, chain)).verifyComplete();
            assertThat(capturedExchange[0].getRequest().getHeaders().getFirst("X-Auth-Username")).isEqualTo("user@example.com");
            assertThat(capturedExchange[0].getRequest().getHeaders().getFirst("X-Auth-Role")).isEqualTo("USER");
        }
        ...
        void shouldPropagateAdminRole() {
            ...
            MockServerWebExchange exchange = MockServerWebExchange.from(
                    MockServerHttpRequest.get("/orders/all").header(HttpHeaders.AUTHORIZATION, "Bearer token-admin").build());

            StepVerifier.create(filter.filter(exchange, chain)).verifyComplete();
            assertThat(capturedExchange[0].getRequest().getHeaders().getFirst("X-Auth-Role")).isEqualTo("ADMIN");
        }
        ...
        void shouldNotSetResponseStatusForValidToken() {
            ...
            MockServerWebExchange exchange = MockServerWebExchange.from(
                    MockServerHttpRequest.get("/products").header(HttpHeaders.AUTHORIZATION, "Bearer token-valido").build());
